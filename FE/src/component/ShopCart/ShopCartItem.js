...
function ShopCartItem(props) {
        ...
        if (event.target.value === "0") {
            ...
            if (event.target.value) {
                ...
                if (res && res.errCode === 0) {
                    ...
                    toast.error(res.errMessage || "Đã xảy ra lỗi khi cập nhật số lượng")
    ...
    let handleDeleteShopCart = async () => {
        ...
        if (res && res.errCode === 0) {
            ...
            toast.error(res.errMessage || "Xóa sản phẩm khỏi giỏ hàng thất bại")
        }
    }
    return (
        <tr>
            ...
            <td style={{ textAlign: 'center' }}>
                <h5 style={{ color: '#ff4674' }}>{CommonUtils.formatter.format(quantity * props.price)}</h5>
            </td>
            {props.isOrder === false &&
                <>
