...
function DetailUserPage(props) {
    ...
    let handleSaveInfor = async () => {
        ...
        if (res && res.errCode === 0) {
            ...
            let userRes = await getDetailUserById(id)
            if (userRes && userRes.errCode === 0) {
                setStateUser(userRes.data)
            }
        } else {
    ...
    let handleSendEmail = async () => {
        ...
        if (res && res.errCode === 0) {
            ...
            toast.error(res.errMessage || "Gửi mã xác nhận thất bại")
        }
    }
    let handleOnChangeImage = async (event) => {
        ...
        if (file.size > 31312281) {
            toast.error("Kích thước tệp phải nhỏ hơn 30MB")
        }
        else {
            ...
            <div className="row">
                <div className="col-md-3 border-right">
                    <div className="d-flex flex-column align-items-center text-center p-3 py-5">
                        ...
                        {inputValues.isActiveEmail === 0 &&
                            <span onClick={() => handleSendEmail()} className="text-verify">Xác minh</span>
                        }
                ...
                <div className="col-md-9 border-right">
                    <div className="p-3 py-5">
                        ...
                        <div className="row mt-2">
                            <div className="col-md-6"><label className="labels">Tên</label><input name="firstName" onChange={(event) => handleOnChange(event)} value={inputValues.firstName} type="text" className="form-control" /></div>
                            <div className="col-md-6"><label className="labels">Họ</label><input name="lastName" onChange={(event) => handleOnChange(event)} value={inputValues.lastName} type="text" className="form-control" /></div>
                        </div>
                        ...
                        <div className="row mt-2">
                            <div className="col-md-3">
                                ...
                                >Tải lên <i className="fas fa-upload"></i></label>

                            </div>
                        </div>
                        <div onClick={() => handleSaveInfor()} className="mt-5 text-center"><button className="btn btn-primary profile-button" type="button">Lưu</button></div>
                    </div>
                </div>
