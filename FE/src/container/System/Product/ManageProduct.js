...
import { getAllProductAdmin, handleBanProductService, handleActiveProductService,deleteProductService  } from '../../../services/userService';
...
const ManageProduct = () => {
    ...
    let handleDeleteProduct = async (id) => {
        if (window.confirm("Bạn có chắc chắn muốn xóa sản phẩm này không?")) {
            ...
            console.log("Delete Product Response:", response); // Kiểm tra phản hồi từ API
                toast.success("Xóa sản phẩm thành công!");
                loadProduct(keyword); // Tải lại danh sách sản phẩm
        }
    };
    return (
                                        ...
                                        return (
                                            <tr key={index}>
                                                ...
                                                <td style={{ width: '12%' }}>
                                                    ...
                                                    &nbsp; &nbsp;
                                                    <span onClick={() => handleDeleteProduct(item.id)} style={{ color: 'red', cursor: 'pointer' }}>Xóa</span>

                                                    &nbsp; &nbsp;
                                                    {item.statusData.code === 'S1' ?
                                                        <span onClick={() => handleBanProduct(item.id)} style={{ color: '#0E6DFE', cursor: 'pointer' }} >Ẩn</span>
                                                        : <span onClick={() => handleActiveProduct(item.id)} style={{ color: '#0E6DFE', cursor: 'pointer' }}   >Kích hoạt</span>
                                                    }
