...
const AddTypeShip = (props) => {
    ...
    let handleSaveTypeShip = async () => {
        if (isActionADD === true) {
            ...
            if (res && res.errCode === 0) {
                toast.success("Thêm loại vận chuyển thành công!")
                setInputValues({
                    ...inputValues,
                    ["type"]: '',
                    ["price"]: ''
            ...
            else toast.error("Thêm loại vận chuyển thất bại!")
        } else {
            ...
            if (res && res.errCode === 0) {
                toast.success("Cập nhật loại vận chuyển thành công!")

            }
            else if (res && res.errCode === 2) {
                toast.error(res.errMessage)
            }
            else toast.error("Cập nhật loại vận chuyển thất bại!")
        }
    }
    ...
    return (
        <div className="container-fluid px-4">
            <h1 className="mt-4">Quản lý loại vận chuyển</h1>


            <div className="card mb-4">
                <div className="card-header">
                    <i className="fas fa-table me-1" />
                    {isActionADD === true ? 'Thêm loại vận chuyển mới' : 'Cập nhật thông tin loại vận chuyển'}
                </div>
                <div className="card-body">
                    <form>
                        <div className="form-row">
                            <div className="form-group col-md-6">
                                <label htmlFor="inputEmail4">Tên loại vận chuyển</label>
                        ...
                        <button type="button" onClick={() => handleSaveTypeShip()} className="btn btn-primary">Lưu</button>
                    </form>
                </div>
            </div>
        </div>
