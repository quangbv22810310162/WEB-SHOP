...
const ManageReceipt = () => {
    ...
    let handleOnClickExport =async () =>{
        ...
        if(res && res.errCode == 0){
            await CommonUtils.exportExcel(res.data,"Danh sách đơn nhập hàng","DanhSachNhapHang")
        }
       
    }
    return (
        <div className="container-fluid px-4">
            <h1 className="mt-4">Quản lý đơn nhập hàng</h1>
                                    ...
                                    dataReceipt.map((item, index) => {
                                        return (
                                            <tr key={index}>
                                                ...
                                                <td>{item.supplierData ? item.supplierData.name : 'N/A'}</td>
                                                <td>{item.supplierData ? item.supplierData.phonenumber : 'N/A'}</td>
                                                <td>{item.userData ? item.userData.firstName + " " + item.userData.lastName : 'N/A'}</td>
