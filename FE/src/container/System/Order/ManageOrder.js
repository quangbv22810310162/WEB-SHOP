...
const ManageOrder = () => {
    ...
    let handleOnClickExport = async () => {
        ...
        if (res && res.errCode == 0) {
            await CommonUtils.exportExcel(res.data, "List orders", "ListOrder")
        }
                                    ...
                                    dataOrder.map((item, index) => {
                                        ...
                                        return (
                                            <tr key={index}>
                                                ...
                                                <td>{item.typeShipData ? item.typeShipData.type : ''}</td>
                                                <td>{item.voucherData ? item.voucherData.codeVoucher : ''}</td>
                                                <td>{item.isPaymentOnlien == 0 ? 'Thanh toán khi nhận hàng' : 'Thanh toán trực tuyến'}</td>
                                                <td>{item.statusOrderData ? item.statusOrderData.value : ''}</td>
