...
let deleteProductById = (productId) => {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            // Tìm và xóa chi tiết sản phẩm
            let productDetails = await db.ProductDetail.findAll({ where: { productId: productId } });
            console.log("Product Details found:", productDetails);

            for (let detail of productDetails) {
                console.log("Deleting Product Detail ID:", detail.id);

                // Xóa các ảnh liên quan đến chi tiết sản phẩm
                await db.ProductImage.destroy({ where: { productdetailId: detail.id } });
                console.log("Deleted images for Product Detail ID:", detail.id);

                // Xóa các kích thước liên quan đến chi tiết sản phẩm
                await db.ProductDetailSize.destroy({ where: { productdetailId: detail.id } });
                console.log("Deleted sizes for Product Detail ID:", detail.id);

                // Xóa chi tiết sản phẩm
                await db.ProductDetail.destroy({ where: { id: detail.id } });
                console.log("Deleted Product Detail ID:", detail.id);
            }
            ...
            console.log("Error:", error); // Log ra lỗi nếu có
            reject(error);
        }
    });
};
