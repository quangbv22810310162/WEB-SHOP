...
let paymentOrderVnpay = (req) => {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            var createDate = process.env.DATE_VNPAYMENT || dateFormat(new Date(), 'yyyymmddHHMMss');
            var orderId = uuidv4();
            ...
            var orderInfo = req.body.orderDescription || "Thanh toán đơn hàng tại Shop";
...
let confirmOrderVnpay = (data) => {
    return new Promise(async (resolve, reject) => {
        try {
            // Bỏ qua việc xác thực, luôn trả về thành công
            console.log("Dữ liệu nhận được từ VNPAY:", data);
